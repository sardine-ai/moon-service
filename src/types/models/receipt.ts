...
export type UpdatedTransactionReceiptResponse = Omit<BaseTransactionReceipt & {state: TransactionState}, 'cost' | 'gasCost' | 'totalCost'>;
export type CompletedTransactionReceiptResponse = BaseTransactionReceipt & {state: TransactionState};

export const buildBundleReceiptResponse = (bundle: Bundle): BundleReceiptResponse => {
...
export const buildTransactionReceiptResponse = (
  ...
  switch (transaction.state) {
    case TransactionState.COMPLETED: {
      return {
        totalCost: Number(transaction.cost || 0) + Number(transaction.gasCost || 0),
...
export const allTransactionsComplete = (
  ...
  return transactionReceipts.every(transactionReceipt => transactionReceipt.state == TransactionState.COMPLETED)
}

export const getBundleReceiptTotalCost = (
  transactionReceipts: TransactionReceipts
) => {
  const totalCost = transactionReceipts.reduce((accumulator, transactionReceipt) => { 
