...
import { Bundle, Transaction, TransactionState, Operation, BaseTransaction } from "../types/models";
import { 
  StoreBundle, 
  StoreTransactions, 
  UpdateTransaction,
...
export const bundleInPrismaToBundle = async (prismaBundle: BundleInPrisma & {transactions: Array<TransactionInPrisma>}): Promise<Bundle> => {
  const transactions = await Promise.all(prismaBundle.transactions.map(prismaTransaction => prismaTransactionToTransaction(prismaTransaction)))
  return {
    id: prismaBundle.id,
    operation: Operation[prismaBundle.operation as keyof typeof Operation] ?? Operation.UNKNOWN,
...
export const transactionStateInPrismaToTransactionState = (
  ...
  return TransactionState[transactionState.toString() as keyof typeof TransactionState] ?? TransactionState.UNDEFINED;
}

export const transactionToPrismaBaseTransaction = (transaction: BaseTransaction): TransactionInPrisma => ({
  id: transaction.id,
...
export const prismaTransactionToTransaction = async (prismaTransaction: TransactionInPrisma): Promise<Transaction> => {
  ...
  return {
    ...
    operation: Operation[prismaTransaction.operation as keyof typeof Operation] ?? Operation.UNKNOWN,
...
export const storeBundleTransactions: StoreTransactions = async (bundle: Bundle) => {
  await prismaClient.$transaction([
    ...
    prismaClient.evmTransaction.createMany({
      data: bundle.transactions.map(transaction => transactionToPrismaEvmTransaction(transaction as Transaction))
    })
  ]);
}
...
export const getTransactionByExecutionId: GetTransactionByExecutionId = async (executionId: string) => {
...
export const getBundleByTransactionExecutionId = async (executionId: string) => {
